...
| 人員 CRUD | 列表（篩選：群組、狀態、員工編號、姓名）、新增、編輯、刪除 | GET/POST/PUT/DELETE `/api/personnel/persons`，GET `by-employee-no/:no` |
...
- **人員列表**：篩選（群組、狀態、員工編號/姓名），表格（員工編號、姓名、群組、狀態、操作），新增/編輯/刪除；操作含「設定門禁權限」。
