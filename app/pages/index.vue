<template>
  ...
  <div class="grid grid-cols-1 gap-4 xl:grid-cols-3 2xl:gap-8">
    ...
    <div class="col-span-2 space-y-4 2xl:space-y-8">
      <!-- Data Cards Section -->
      <div
        class="overflow-hidden rounded-2xl border-2 border-white/80 bg-white/30"
      >
        <div class="grid h-full grid-cols-1 xl:grid-cols-12">
          ...
          <!-- Environmental Card -->
          <EnvironmentCard
            class="col-span-1 xl:col-span-5"
    ...
    <div class="col-span-1 grid grid-rows-12">
      <div
        class="row-span-12 overflow-hidden rounded-2xl border-2 border-white/80 bg-white/30"
      >
        <!-- Building Image Card -->
        <BuildingCard />
      </div>
...
const transformSensorData = (raw: number[]) => {
  ...
  sensorData.humidity = Number(
    (raw[SENSOR_ADDRESSES.HUMIDITY] / 10).toFixed(1)
  );
  sensorData.temperature = Number(
    (raw[SENSOR_ADDRESSES.TEMPERATURE] / 10).toFixed(1)
  );
...
const loadSensorDevice = async () => {
  try {
    ...
    const errorMsg =
      error instanceof Error ? error.message : "載入感測器設備失敗";
...
const loadSensorData = async () => {
  ...
  try {
    // 直接調用 Modbus API，無需額外的 composable 層
...
const aqiData = computed(() => ({
  ...
  metrics: [
    ...
    {
      label: "TVOC",
      value: toFixedNumber(sensorData.tvoc, 3),
      unit: "ppm",
      icon: "TVOC",
    },
    {
      label: "HCHO",
      value: toFixedNumber(sensorData.hcho),
      unit: "ppm",
      icon: "HCHO",
    },
  ],
