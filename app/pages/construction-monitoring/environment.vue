	...
	try {
		...
		if (isDayRange) {
			const [summaryRes, detailRes] = await Promise.all([
				environmentApi.getReadingsAggregated(loc.id, { bucket: "hour", startTime: startDate, endTime: endDate }),
				environmentApi.getReadings(loc.id, { startTime: startDate, endTime: endDate, limit: 500 }),
			])
			simulationReadingsSummary.value = summaryRes.readings ?? []
			simulationReadingsDetail.value = detailRes.readings ?? []
		} else if (isWeekRange) {
			const [summaryRes, detailRes] = await Promise.all([
				environmentApi.getReadingsAggregated(loc.id, { bucket: "day", startTime: startDate, endTime: endDate }),
				environmentApi.getReadingsAggregated(loc.id, { bucket: "hour", startTime: startDate, endTime: endDate }),
			])
			simulationReadingsSummary.value = summaryRes.readings ?? []
			simulationReadingsDetail.value = detailRes.readings ?? []
		} else {
			const result = await environmentApi.getReadings(loc.id, { startTime: startDate, endTime: endDate, limit: 500 })
			simulationReadingsSummary.value = []
