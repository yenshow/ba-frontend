...
/** DB 以 central profile 種子時，工地前端顯示名對齊授權管理 */
const PARENT_DISPLAY_NAMES: Record<string, string> = {
	"system.people_counting": "人流統計",
};
...
const withDisplayName = (def: PermissionDefinition): PermissionDefinition => {
	if (def.parent_id != null) return def;
	const name = PARENT_DISPLAY_NAMES[def.code];
...
const filterVisibleDefinitions = (
	...
	const inProfile = defs.filter(d => PROFILE_CODES.has(d.code)).map(withDisplayName);
