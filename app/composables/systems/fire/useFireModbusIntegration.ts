...
export const useFireModbusIntegration = (zonesRef: Ref<FireZone[]>) => {
	...
	const patchOptimisticManualAlarm = (systemId: string, rule?: ManualIssueRuleTriggerPayload) => {
		patchOptimisticManualAlarmForZones(statusItems, zonesRef.value, systemId, "fire", rule)
	}
	...
	const loadStatusSnapshot = async (options?: StatusSnapshotFetchOptions) => {
		...
		await race.runSnapshotLoad(options, async ({ isStale }) => {
			try {
				const result = await fireApi.getStatus({ syncAlerts })
				if (isStale()) return
