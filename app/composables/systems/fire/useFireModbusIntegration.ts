...
import type { FireLocation, FireStatusItem, FireZone } from "~/types/fire"
...
type BatchRequest = {
	...
	locationId: string
	locationName: string
	equipmentKind: FireStatusItem["equipmentKind"]
	viewCategory: string
	bitKey: string
...
const deriveUiStatusForFireItem = (item: FireStatusItem): FireStatusItem["uiStatus"] => {
	const raw = item.raw || {}
...
export const useFireModbusIntegration = (fireZones: Ref<FireZone[]>) => {
				...
				for (const bitKey of Object.keys(points)) {
					...
					if (!deviceConfig) {
						reqs.push({
							...
							viewCategory: String(loc.viewCategory || "sprinkler"),
							bitKey,
	...
	const loadStatusSnapshot = async () => {
		...
		inflightSnapshot = (async () => {
			...
			const baseItemsBySystemId = new Map<string, FireStatusItem>()
			...
			for (const [requestKey, groupRequests] of grouped.entries()) {
				...
				// 選項 A：hot 期間要「即時」就不能被前端 value-cache 擋住
				...
				if (cached && now - cached.timestamp < REQUEST_CACHE_TTL) {
					if (cached.ok) {
						...
						groupRequests.forEach((gr) => {
							...
							it.uiStatus = "warning"
							it.error = it.error || cached.error || "無法讀取消防設備資料"
						})
					}
					continue
				}
			...
			if (batchPayload.length > 0) {
				try {
					...
					for (const [requestKey, groupRequests] of grouped.entries()) {
						...
						const msg = String((r as any).error || "無法讀取消防設備資料")
					...
					for (const [requestKey, groupRequests] of grouped.entries()) {
						...
						groupRequests.forEach((gr) => {
							...
							it.raw[gr.bitKey] = undefined
							it.uiStatus = "warning"
							it.error = it.error || msg || "無法讀取消防設備資料"
						})
					}
				}
			}

			const out: FireStatusItem[] = Array.from(baseItemsBySystemId.values()).map((it) => {
			...
			// 選項 A：只有在「警報/異常」或「剛發生狀態變動」時，短時間內改用 noCache 新鮮讀取
			for (const it of out) {
	...
	watch(
		() => fireZones.value,
		async () => {
			// 精簡：避免 systemHotUntil / lastRawBySystemId 因 zone 變更而無限成長
			const activeSystemIds = new Set<string>()
			for (const z of fireZones.value || []) {
