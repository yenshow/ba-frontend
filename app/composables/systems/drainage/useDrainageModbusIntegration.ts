...
export const useDrainageModbusIntegration = (zonesRef: Ref<DrainageZone[]>) => {
	...
	const patchOptimisticManualAlarm = (systemId: string, rule?: ManualIssueRuleTriggerPayload) => {
		patchOptimisticManualAlarmForZones(statusItems, zonesRef.value, systemId, "drainage", rule)
	}
	...
	const loadStatusSnapshot = async (options?: StatusSnapshotFetchOptions) => {
		...
		await race.runSnapshotLoad(options, async ({ isStale }) => {
			try {
				const result = await drainageApi.getStatus({ syncAlerts })
				if (isStale()) return
