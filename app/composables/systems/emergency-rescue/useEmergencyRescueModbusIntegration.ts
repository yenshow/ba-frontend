...
export const useEmergencyRescueModbusIntegration = (zonesRef: Ref<EmergencyRescueZone[]>) => {
	...
	const patchOptimistic = (systemId: string, uiStatus: EmergencyRescueStatusItem["uiStatus"]) => {
	...
	const loadStatusSnapshot = async (options?: StatusSnapshotFetchOptions) => {
		...
		await race.runSnapshotLoad(options, async ({ isStale }) => {
			try {
				const result = await emergencyRescueApi.getStatus({ syncAlerts })
				if (isStale()) return
