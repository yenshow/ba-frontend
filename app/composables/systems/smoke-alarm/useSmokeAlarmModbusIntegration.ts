...
export const useSmokeAlarmModbusIntegration = (zonesRef: Ref<SmokeAlarmZone[]>) => {
	...
	const patchOptimistic = (systemId: string, uiStatus: SmokeAlarmStatusItem["uiStatus"]) => {
	...
	const loadStatusSnapshot = async (options?: StatusSnapshotFetchOptions) => {
		...
		await race.runSnapshotLoad(options, async ({ isStale }) => {
			try {
				const result = await smokeAlarmApi.getStatus({ syncAlerts })
				if (isStale()) return
