...
export function useLightingLocationValidation() {
	...
	const validateModbusAddress = (
		...
		if (!deviceId || deviceId === 0) {
			return null; // 沒有選擇設備時不驗證地址
		}

		if (address === undefined || address === null) {
			return "Modbus 地址不能為空";
		}

		if (address < 0) {
			return "Modbus 地址不能為負數";
		}

		if (address > 65535) {
			return "Modbus 地址不能超過 65535";
		}
	...
	const validateModbusType = (
		...
		if (!deviceId || deviceId === 0) {
			return null; // 沒有選擇設備時不驗證類型
		}

		if (!type) {
			return "Modbus 類型不能為空";
		}

		if (type !== "DI" && type !== "DO") {
			return "Modbus 類型必須是 DI 或 DO";
		}
