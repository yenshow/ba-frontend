...
import type { useDeviceApi } from "~/composables/systems/devices/useDeviceApi"
import type { useAccessControlApi } from "~/composables/systems/accessControl/useAccessControlApi"
...
export type PersonDialogSnapshot = {
	employeeNo: string
	fullName: string
...
/** 人員新增／編輯 Dialog 表單狀態與儲存流程 */
export const usePersonnelPersonForm = (params: {
	...
	persons: Ref<Person[]>
	personsOffset: Ref<number>
	loadPersons: () => Promise<void>
	getFaceImageSrc: (url: string | null | undefined) => string | null
}) => {
	const { personnelApi, deviceApi, accessControlApi, toast, handleApiError, persons, personsOffset, loadPersons, getFaceImageSrc } = params

	const showPersonDialog = ref(false)
	const personDialogSection = ref<PersonnelPersonDialogSection>("form")
	...
	const requestClosePersonDialog = () => {
		if (hasUnsavedPersonChanges.value) {
			personCloseConfirm.show(FORM_UNSAVED_CLOSE_CONFIRM)
			return
		}
		closePersonDialog()
	}
	...
	const openPersonCreate = (section: PersonnelPersonDialogSection = "form") => {
		editingPerson.value = null
		personDialogSection.value = section
	...
	const editPerson = async (p: Person, section: PersonnelPersonDialogSection = "form") => {
		let full: Person
		try {
	...
	watch(showPersonDialog, (v) => {
		...
		personDialogSection.value = "form"
	})
