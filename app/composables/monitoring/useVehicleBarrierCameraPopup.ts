...
const resolveEventLabel = (payload: VehicleAccessWsPayload): string => {
	const raw = String(payload.eventLabel || "").trim()
	if (raw) return raw
	if (payload.source === "manual") return "手動開閘"
...
export const useVehicleBarrierCameraPopup = () => {
	...
	const enqueue = async (payload: VehicleAccessWsPayload) => {
	...
	useWebSocketEventSubscription(
		...
		(...args: unknown[]) => {
			void enqueue((args[0] ?? {}) as VehicleAccessWsPayload)
		},
