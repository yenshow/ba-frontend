...
export const useWebSocket = () => {
	...
	const connect = () => {
		...
		if (globalSocket?.connected || status.value === "connecting") {
			if (process.dev) {
				console.log("[WebSocket] 連接已存在，跳過重複連接");
			}
			return;
		}
		...
		try {
			...
			globalSocket.on("connect", () => {
				...
				if (process.dev) {
					console.log("[WebSocket] 連接成功:", globalSocket?.id);
				}
			});
			...
			globalSocket.on("reconnect_attempt", (attemptNumber: number) => {
				if (process.dev) {
					console.log(`[WebSocket] 重連嘗試 ${attemptNumber}...`);
				}
			});
			...
			globalSocket.on("reconnect", (attemptNumber: number) => {
				...
				if (process.dev) {
					console.log(`[WebSocket] 重連成功 (嘗試 ${attemptNumber} 次)`);
				}
			});
	...
	const on = (event: string, callback: Function) => {
		if (!globalSocket) {
			if (process.dev) {
				console.warn(`[WebSocket] 嘗試監聽事件 ${event}，但連接尚未建立`);
			}
			return;
		}
