...
export const useWebSocketMonitor = () => {
	...
	/**
	 * 設置 WebSocket 事件監聽器
	 * @param configs - 監聽器配置陣列
	 */
	const setupListeners = <T = any>(configs: WebSocketListenerConfig<T>[]) => {
		...
		configs.forEach(({ event, handler, logMessage }) => {
			...
			const wrappedHandler = (data: T) => {
				if (process.dev && logMessage) {
					console.log(`[WebSocketMonitor] ${logMessage(data)}`);
				}
				handler(data);
			};
		...
		if (process.dev) {
			console.log(`[WebSocketMonitor] 已設置 ${configs.length} 個事件監聽器`);
		}
	};
	...
	const removeListeners = (events?: string[]) => {
		...
		if (process.dev) {
			console.log(
				`[WebSocketMonitor] 已移除 ${events ? events.length : "所有"} 個事件監聽器`
			);
		}
	};
