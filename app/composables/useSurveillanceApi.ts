...
export const useSurveillanceApi = () => {
	...
	const getCameraDevices = async (): Promise<Device[]> => {
		try {
			...
			console.error("[Surveillance API] 獲取攝影機設備失敗:", error);
			throw error;
		}
	};
	...
	const getCamerasWithStreamInfo = async (): Promise<SurveillanceCamera[]> => {
		try {
			...
			console.error("[Surveillance API] 獲取攝影機與串流資訊失敗:", error);
			throw error;
		}
	};
	...
	const startCameraStream = async (deviceId: number): Promise<RTSPStreamInfo> => {
		try {
			...
			console.error(`[Surveillance API] 啟動攝影機串流失敗 (設備 ID: ${deviceId}):`, error);
			throw error;
		}
	};
	...
	const stopCameraStream = async (deviceId: number): Promise<void> => {
		try {
			...
			if (!stream) {
				console.warn(`[Surveillance API] 未找到對應的串流 (設備 ID: ${deviceId})`);
				return;
			}
			...
			console.error(`[Surveillance API] 停止攝影機串流失敗 (設備 ID: ${deviceId}):`, error);
			throw error;
		}
	};
	...
	const getMultipleCameraStreamStatus = async (deviceIds: number[]): Promise<CameraStreamStatus[]> => {
		try {
			...
			console.error("[Surveillance API] 批量獲取串流狀態失敗:", error);
			return [];
		}
	};
