...
export const useAccessGate = () => {
	...
	const useCanLoadFeature = (featureKey: FeatureKey | null, options?: { permissionCode?: string }) =>
		computed(() => canLoadFeature(featureKey, options));

	/** WS 訂閱：registry + license 就緒且通過 canLoadFeature */
	const useWsModuleGate = (featureKey: FeatureKey | null, options?: { permissionCode?: string }) => {
