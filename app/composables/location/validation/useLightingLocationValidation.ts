...
export function useLightingLocationValidation() {
	...
	const validateLightingLocation = (
		...
		if (location.name && allLocations.length > 0) {
			const isDuplicate = checkDuplicateLocationName(location.name, allLocations, location.id)
			if (isDuplicate) {
		...
		if (location.deviceId && location.deviceId > 0) {
			const typeError = validateModbusType(location.modbus?.points?.[0]?.type, location.deviceId)
	...
	const validateZoneLocations = (
		...
		locations.forEach((location, index) => {
			...
			if (!result.isValid) {
				allErrors.push(
					...result.errors.map((err) => `點位「${location.name || `點位 ${index + 1}`}」: ${err}`)
				)
			}
			if (result.warnings.length > 0) {
				allWarnings.push(
					...result.warnings.map(
						(warn) => `點位「${location.name || `點位 ${index + 1}`}」: ${warn}`
					)
				)
			}
		})
