			...
			>
				...
				>
					...
					<p class="text-sm text-white/70">
						此處為「可進出之地點」（可多選），不是門禁設備列表。門禁設備請在「設備管理」新增；地點需在「人流統計」中建立並綁定入口／出口設備後，才會出現在下方。門禁事件由後端自動向設備訂閱，不需在設備上設定 HTTP 監聽主機。
					</p>
					...
					<div v-else class="space-y-2">
						...
						>
							...
							/>
							<span class="text-sm text-white/90 2xl:text-base">{{ loc.zone_name }} — {{ loc.name }}</span>
						</label>
						<div
					...
					<footer class="mt-2 flex gap-3 2xl:gap-4">
						...
						<button type="button" class="btn-primary" :disabled="isSaving || !person" @click="handleSave">
							{{ isSaving ? "儲存中..." : "儲存" }}
						</button>
					</footer>
				</div>
