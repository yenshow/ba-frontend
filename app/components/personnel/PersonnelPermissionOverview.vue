<template>
	<div class="space-y-4 text-sm text-white/85 2xl:text-base">
		...
		<section v-if="ladderCardSummary">
			<h4 class="mb-2 font-medium text-white">梯控卡</h4>
			<p class="text-white/75">
				卡號 <span class="font-mono">{{ ladderCardSummary.cardNo }}</span>
				<span v-if="ladderCardSummary.floorCount > 0" class="ms-2 text-white/55">
					（預設 {{ ladderCardSummary.floorCount }} 個樓層）
				</span>
			</p>
			<NuxtLink
				v-if="showElevatorLink"
				to="/access-control/elevator"
				class="mt-2 inline-block rounded-md bg-white/10 px-2.5 py-1 text-xs text-cyan-200 hover:bg-white/15 2xl:text-sm"
			>
				前往樓層管理
			</NuxtLink>
		</section>
...
const ladderCardSummary = computed(() => {
	const card = props.person.ladder_card
	if (!card?.card_no?.trim()) return null
	const floors = card.floors
	let floorCount = 0
	if (Array.isArray(floors)) floorCount = floors.length
	else if (floors && typeof floors === "object" && floors.byLocation) {
		floorCount = Object.values(floors.byLocation).reduce((sum, arr) => sum + (arr?.length ?? 0), 0)
	}
	return { cardNo: card.card_no.trim(), floorCount }
})

const showVehicleLink = computed(() => canLoadFeature("vehicle_access"))
const showElevatorLink = computed(() => canLoadFeature("elevator"))
