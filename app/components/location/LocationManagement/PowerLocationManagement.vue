...
import PowerLocationFields from "../LocationFormFields/PowerLocationFields.vue"
...
interface GroupRow {
	...
	viewCategory: string
	displayLabel: string
	items: { loc: PowerLocation; globalIndex: number }[]
}
...
const categoryPlaceholder = computed(() => "例如：備援、主機房、B1")

const normalizeCategory = (loc: PowerLocation) => (loc.viewCategory ?? "").trim()
...
const groupedLocations = computed((): GroupRow[] => {
	...
	locations.value.forEach((loc, globalIndex) => {
		...
		const displayLabel = getPowerViewCategoryDisplayLabel(raw)
		if (!map.has(key)) {
...
const getStableItemKey = (item: { loc: PowerLocation; globalIndex: number }) =>
	getLocationUiKey({
...
const handleLocationUpdate = (locationIndex: number, updatedLocation: PowerLocation) => {
