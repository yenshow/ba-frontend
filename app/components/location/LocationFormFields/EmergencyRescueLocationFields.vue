...
watch(
	...
	(next) => {
		...
		sosAddress.value = Number(localLocation.value.statusPoints?.sos?.address) || 0
	},
...
const otherTupleKeys = computed(() => {
	...
	for (let i = 0; i < props.allLocations.length; i++) {
		...
		const address = Number(loc.statusPoints?.sos?.address)
...
const hasDuplicateAddress = computed(() => {
	...
	const k = tupleKey({ deviceId: Number(localLocation.value.deviceId), address: sosAddress.value })
...
const handleChange = () => {
	...
	if (next.deviceId && next.deviceId > 0) {
		next.statusPoints = { ...(next.statusPoints || {}) }
		next.statusPoints.sos = {
			registerType: "discrete",
			address: sosAddress.value,
		}
	} else {
		next.statusPoints = {}
	}
