	...
	<div class="flex min-w-0 flex-1 flex-col gap-3">
		...
		<div :class="fieldLabelClass">
			...
			<div v-else class="grid grid-cols-1 gap-2">
				<label
					...
					:class="[
						selectCardBaseClass,
						selectedDeviceId === device.id && selectCardSelectedClass,
					]"
				>
					<input
						type="radio"
		...
		<div v-if="selectedDeviceId" :class="fieldLabelClass">
			...
			<div v-if="floorCount != null" class="flex min-w-0 flex-col gap-2">
				<span>樓層名稱</span>
				<div
					class="grid max-h-56 grid-cols-2 gap-2 overflow-y-auto rounded-lg border border-white/10 bg-white/5 p-2"
					role="table"
					aria-label="樓層名稱列表"
				>
					<div
		...
		<div :class="fieldLabelClass">
			...
			<div class="grid grid-cols-2 gap-2 lg:grid-cols-3">
				...
				>
					...
					/>
					<span class="text-xs text-white/90 2xl:text-sm">{{ ELEVATOR_LOG_COLUMN_LABELS[key] }}</span>
				</label>
			</div>
		</div>
	</div>
...
const emptyHintClass = "rounded-lg border border-dashed border-white/20 px-3 py-2 text-sm text-white/50"
