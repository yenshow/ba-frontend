		...
		<div class="flex items-end justify-between h-[88px] px-12 pb-3 2xl:h-[96px] 2xl:px-16 2xl:pb-4">
			...
			<div class="flex items-center space-x-6 2xl:space-x-8">
				...
				<div class="relative flex items-center" ref="userMenuRef">
					...
					<!-- Dropdown Menu：使用者資訊區、使用者管理(不含 icon)、登入登出 -->
					<Transition
						enter-active-class="transition ease-out duration-100"
						enter-from-class="transform opacity-0 scale-95"
						enter-to-class="transform opacity-100 scale-100"
					...
					>
						...
						>
							...
							<!-- Menu Items -->
							<div class="py-1">
								<!-- 使用者管理 (不含 icon，僅管理員顯示) -->
								<NuxtLink
									v-if="isAdmin"
									to="/core/users"
									@click="closeUserMenu"
									class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 2xl:text-base"
								>
									使用者管理
								</NuxtLink>
